...
const getCartItems = (userId) => (dispatch) => {
  ...
  return axios(`https://ecommerse-server-j8hm.onrender.com/cart/${userId}`, {
    headers: {
...
const postCartItems = (item, userId) => (dispatch) => {
  ...
  return axios
    .post(`https://ecommerse-server-j8hm.onrender.com/cart/${userId}`, item)
    .then((res) => {
...
const removeCartItem = (userId, productId, toast) => (dispatch) => {
  ...
  return axios
    .delete(
      `https://ecommerse-server-j8hm.onrender.com/cart/${userId}/${productId}`
    )
    .then((res) => {
